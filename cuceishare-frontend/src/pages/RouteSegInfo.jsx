...
export default function RouteSegInfo() {
  ...
  // Apuntes (solo “Abrir archivo”, sin sugeridos)
    ...
    (async () => {
      ...
      try {
        const [sumRes, resMe] = await Promise.all([
          axios.get(`${API}/api/seginf/route/summary`, { headers: HEADERS, params: { _t: Date.now() } }),
          axios.get(`${API}/api/seginf/results/me`,    { headers: HEADERS, params: { _t: Date.now() } }),
        ]);
        if (!alive) return;
  ...
  // Apuntes vinculados por bloque (sin `q`, con fallback si el backend devuelve 0)
  useEffect(() => {
    let alive = true;
    if (!openBlock) return;

    const MATERIA = 'seginf';
    const ALIASES = [
      'seguridad', 'seguridad-info', 'seguridad-informacion', 'seguridad_informacion', 'seg-info'
    ];

    (async () => {
      setNotesLoading(true); setNotesErr(''); setRelatedNotes([]);
      try {
        // 1) Estricto: blockId + materia principal
        let step = 'strict';
        ...
        // 2) Fallback: solo por blockId (sin materia)
        if (notes.length === 0) {
          step = 'block-only';
        ...
        // 3) Fallback: probar alias de materia
        if (notes.length === 0) {
        ...
        console.debug('[SEGINF] apuntes vinculados →', { blockId: openBlock, step, count: notes.length });
      } catch (e) {
        if (!alive) return;
      ...
      <header className="sticky top-0 z-40 border-b bg-white/80 backdrop-blur supports-[backdrop-filter]:bg-white/60">
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between gap-4">
            ...
            <div className="truncate">
              <nav className="text-xs text-slate-500" aria-label="Breadcrumb">
                <ol className="flex items-center gap-1">
                  ...
                  <li className="hover:text-slate-700 cursor-default">Seguridad de la Información</li>
                  <li className="text-slate-400">/</li>
          ...
          <div className="hidden md:flex items-center gap-2">
            <Link to="/pre-eval/seginf" className="inline-flex items-center gap-2 px-3 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white text-sm shadow-sm">
              📊 Ir a la pre-evaluación
            </Link>
            <Link to="/buscar?materia=seginf" className="inline-flex items-center gap-2 px-3 py-2 rounded-xl bg-white border hover:bg-slate-50 text-slate-800 text-sm shadow-sm">
              🔎 Explorar apuntes
            </Link>
          </div>
        </div>
        ...
        <div className="h-full flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto p-4">
            ...
            <section className="mb-6">
              ...
              {notesLoading && <div className="text-slate-600 mb-2">Buscando apuntes…</div>}
              {notesErr && <div className="p-3 rounded-xl border bg-rose-50 text-rose-800 mb-2">{notesErr}</div>}
