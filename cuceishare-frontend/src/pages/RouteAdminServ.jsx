...
export default function RouteAdminServ() {
    ...
    (async () => {
      ...
      try {
        const [sumRes, resMe] = await Promise.all([
          axios.get(`${API}/api/aserv/route/summary`, { headers: HEADERS, params: { _t: Date.now() } }),
          axios.get(`${API}/api/aserv/results/me`,    { headers: HEADERS, params: { _t: Date.now() } }),
        ]);
        if (!alive) return;
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        const { data } = await axios.get(`${API}/api/aserv/route/resources`, {
          headers: HEADERS,
            ...
            <div className="truncate">
              <nav className="text-xs text-slate-500" aria-label="Breadcrumb">
                <ol className="flex items-center gap-1">
                  ...
                  <li className="hover:text-slate-700 cursor-default">Administración de Servidores</li>
                  <li className="text-slate-400">/</li>
                  <li className="text-slate-700 font-medium truncate">Mi ruta</li>
                </ol>
              </nav>
              <h1 className="text-lg md:text-xl font-bold tracking-tight text-slate-900">
                Ruta de aprendizaje — <span className="text-emerald-700">Administración de Servidores</span>
              </h1>
      ...
      <main className="max-w-7xl mx-auto px-4 py-6 space-y-6">
        ...
        {err && !loading && (
          <div className="p-4 rounded-xl bg-rose-50 border text-rose-700">
            ⚠️ {err}
            <button onClick={() => window.location.reload()} className="ml-3 px-3 py-1.5 rounded-lg bg-white border hover:bg-slate-50">
              Reintentar
            </button>
          </div>
        )}
          ...
          <>
            ...
            {!(totals.total > 0) && (
              <div className="rounded-2xl border bg-white shadow-sm p-5">
                <div className="text-lg font-bold mb-1">Aún no tienes resultados en Administración de Servidores</div>
                      ...
                      <div key={b.block_id || i} className="rounded-xl border p-4">
                        ...
                        <div className="mt-3 text-sm">
                          {pct >= 80 ? (
                            ...
                            <div className="text-amber-700">🟡 Intermedio. Repasa servicios clave y configura entornos.</div>
                          ) : (
                            <div className="text-rose-700">🔴 Débil. Inicia con fundamentos de Linux, redes y servicios básicos.</div>
                          )}
                        </div>
        ...
        <div className="h-full flex flex-col">
          ...
          <div className="px-4 py-3 border-t">
            <div className="flex justify-end">
              <button onClick={() => setOpenBlock(null)} className="px-3 py-2 rounded-lg border bg-white hover:bg-slate-50">
                Cerrar
              </button>
            </div>
          </div>
