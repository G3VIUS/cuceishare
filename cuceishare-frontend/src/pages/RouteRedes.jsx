...
export default function RouteRedes() {
  ...
  // Carga principal (summary + results) con fallback de totales
  useEffect(() => {
    let alive = true;
    if (!isAuthed) return;
    ...
    (async () => {
      ...
      try {
        const [sumRes, resMe] = await Promise.all([
          axios.get(`${API}/api/redes/route/summary`, { headers: HEADERS, params: { _t: Date.now() } }),
          axios.get(`${API}/api/redes/results/me`,    { headers: HEADERS, params: { _t: Date.now() } }),
        ]);
        if (!alive) return;
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        const { data } = await axios.get(`${API}/api/redes/route/resources`, {
          headers: HEADERS,
  ...
  // Apuntes relacionados (plataforma) por bloque — sin sugeridos, con fallbacks
  useEffect(() => {
    let alive = true;
    if (!openBlock) return;
    ...
    const MATERIA = 'redes';
    const ALIASES = [
      'redes', 'redes-2', 'redes_de_computadoras', 'redes-de-computadoras', 'networking'
    ];

    (async () => {
      setNotesLoading(true); setNotesErr(''); setRelatedNotes([]);
      try {
        // 1) Estricto: blockId + materia
        let step = 'strict';
        let resp = await axios.get(`${API}/apuntes`, {
          headers: HEADERS,
          params: { blockId: openBlock, materia: MATERIA, _t: Date.now() },
        });
        let notes = normalizeNotes(resp.data);

        // 2) Solo blockId (por si los apuntes no guardaron materia)
        if (notes.length === 0) {
          step = 'block-only';
          resp = await axios.get(`${API}/apuntes`, {
            headers: HEADERS,
            params: { blockId: openBlock, _t: Date.now() },
          });
          notes = normalizeNotes(resp.data);
        }

        // 3) Probar alias de materia
        if (notes.length === 0) {
          for (const alias of ALIASES) {
            step = `alias:${alias}`;
            resp = await axios.get(`${API}/apuntes`, {
              headers: HEADERS,
              params: { blockId: openBlock, materia: alias, _t: Date.now() },
            });
            notes = normalizeNotes(resp.data);
        ...
        console.debug('[REDES] apuntes vinculados →', { blockId: openBlock, step, count: notes.length });
      } catch (e) {
        if (!alive) return;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-50 to-slate-100">
            ...
            <div className="truncate">
              <nav className="text-xs text-slate-500" aria-label="Breadcrumb">
                <ol className="flex items-center gap-1">
                  ...
                  <li className="hover:text-slate-700 cursor-default">Redes</li>
                  <li className="text-slate-400">/</li>
                  <li className="text-slate-700 font-medium truncate">Mi ruta</li>
                </ol>
              </nav>
              <h1 className="text-lg md:text-xl font-bold tracking-tight text-slate-900">
                Ruta de aprendizaje — <span className="text-emerald-700">Redes</span>
              </h1>
          ...
          <>
            ...
            {!(totals.total > 0) && (
              <div className="rounded-2xl border bg-white shadow-sm p-5">
                <div className="text-lg font-bold mb-1">Aún no tienes resultados para Redes</div>
                <p className="text-slate-600 mb-3">
                  Realiza la <span className="font-semibold">pre-evaluación</span> para generar tu ruta de estudio.
                </p>
                <div className="flex gap-2">
                  <Link to="/pre-eval/redes" className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white">📊 Empezar pre-evaluación</Link>
                  <Link to="/buscar?materia=redes" className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-white border hover:bg-slate-50">🔎 Explorar apuntes</Link>
                </div>
              </div>
            )}
            ...
            <section className="flex flex-wrap items-center gap-2">
              <Link to="/pre-eval/redes" className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white">📊 Abrir pre-evaluación</Link>
              <Link to="/buscar?materia=redes" className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-white border hover:bg-slate-50">🔎 Buscar apuntes</Link>
      ...
      {/* Slide-over fondo */}
      {openBlock && (
        <div className="fixed inset-0 z-40 bg-black/40" onClick={() => setOpenBlock(null)} aria-hidden="true" />
      )}
