...
export default function RouteGeneric() {
  ...
  const [blocks, setBlocks] = useState([]); // {block_id, block_title, block_code, total_option, correct_option}
  ...
  // Carga inicial: resumen y totales
  useEffect(() => {
    let mounted = true;
    setLoading(true);
    setErr('');
    ...
    (async () => {
      try {
        ...
        // Seleccionar por defecto el bloque más débil
        const decorated = (summary.blocks || []).map(b => ({
          ...b,
          _pct: pct(Number(b.correct_option || 0), Number(b.total_option || 0)),
        }));
        decorated.sort((a, b) => (a._pct - b._pct) || (a.block_order - b.block_order || 0));
  ...
  // Cargar detalle de bloque (tab Resumen)
  useEffect(() => {
  ...
  // Cargar materiales del bloque (tab Material)
  useEffect(() => {
  ...
  // Cargar recomendaciones (tab Recom)
  useEffect(() => {
  ...
  async function startAdaptive() {
    const withPct = (blocks || []).map(b => ({
      ...b,
      _pct: pct(Number(b.correct_option || 0), Number(b.total_option || 0)),
    }));
    withPct.sort((a, b) => (a._pct - b._pct));
    const candidates = withPct.filter(b => Number(b.total_option || 0) > 0);
    const pick = (candidates.length ? candidates : withPct).slice(0, 3).map(b => b.block_id);
  ...
  return (
    <div className="max-w-7xl mx-auto">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mb-4">
        <div>
          ...
          <p className="text-sm text-gray-600">
            Ruta personalizada {sessionId ? <span className="text-gray-500">• sesión {sessionId.slice(0, 8)}…</span> : null}
          </p>
        </div>
        <div className="flex gap-2">
          <button
            ...
            title="Crea una práctica tomando 2–3 bloques con menor precisión global"
          >
      ...
      {/* KPI: Global + Bloque seleccionado */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-2">
        <div className="p-3 border rounded-lg md:col-span-2">
          ...
          <div className="grid grid-cols-3 gap-3 mt-2">
            <div className="p-2 rounded border" title="Preguntas correctas (global). Contamos tu último intento por pregunta.">
            ...
            <div className="p-2 rounded border" title="Preguntas que intentaste (global).">
          ...
          <p className="text-xs text-gray-500 mt-2">
            Basado en tu <strong>última sesión</strong> para esta materia. Contamos tu <strong>último intento</strong> por cada pregunta.
          </p>
        </div>
          ...
          {!selectedBlock ? (
            ...
            <>
              ...
              <div className="grid grid-cols-3 gap-2 mt-2">
                ...
                <div className="p-2 rounded border" title="Precisión de este bloque.">
        ...
        <aside className="col-span-12 md:col-span-4">
          <div className="border rounded-lg">
            <div className="px-3 py-2 border-b bg-gray-50 rounded-t-lg flex justify-between items-center">
              <div className="font-medium">Bloques (cifras por bloque)</div>
        ...
        <main className="col-span-12 md:col-span-8">
          ...
          {activeTab === 'resumen' && (
            <section className="space-y-3">
              {!selectedBlock ? (
                <div className="p-3 border rounded text-sm text-gray-600">Selecciona un bloque en la izquierda para ver el detalle.</div>
              ) : blockDetail.loading ? (
                ...
                <>
                  ...
                  <div className="p-3 border rounded">
                    <div className="text-sm text-gray-500 mb-2">
                      Diagnóstico del bloque (últ. intento por pregunta): {materials.data.diagnosis.accuracy}% ({materials.data.diagnosis.correctas}/{materials.data.diagnosis.total})
                    </div>
