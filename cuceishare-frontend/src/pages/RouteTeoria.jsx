...
export default function RouteTeoria() {
  ...
  // Apuntes vinculados por bloque — SIN sugeridos (no enviar `q`)
  useEffect(() => {
    let alive = true;
    if (!openBlock) return;
    (async () => {
      ...
      try {
        const { data } = await axios.get(`${API}/apuntes`, {
          headers: HEADERS,
          params: { blockId: openBlock, materia: 'teoria', _t: Date.now() },
        });
        if (!alive) return;
        setRelatedNotes(normalizeNotes(data));
        console.debug('[TEO] apuntes vinculados →', { blockId: openBlock, count: Array.isArray(data?.items ?? data) ? (data.items?.length ?? data.length) : 0 });
      } catch (e) {
        if (!alive) return;
            ...
            <div className="truncate">
              <nav className="text-xs text-slate-500" aria-label="Breadcrumb">
                <ol className="flex items-center gap-1">
                  <li>Aprendizaje</li><li className="text-slate-400">/</li><li>Teoría de la Computación</li><li className="text-slate-400">/</li><li className="font-medium">Mi ruta</li>
                </ol>
              </nav>
              <h1 className="text-lg md:text-xl font-bold tracking-tight text-slate-900">
                Ruta de aprendizaje — <span className="text-emerald-700">Teoría de la Computación</span>
              </h1>
          ...
          <>
            ...
            {!(totals.total > 0) && (
              <div className="rounded-2xl border bg-white shadow-sm p-5">
                <div className="text-lg font-bold mb-1">Aún no tienes resultados para Teoría de la Computación</div>
                ...
                <div className="flex gap-2">
                  <Link to="/pre-eval/teoria" className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white">📊 Empezar pre-evaluación</Link>
                  <Link to="/buscar?materia=teoria" className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-white border hover:bg-slate-50">🔎 Explorar apuntes</Link>
                </div>
              </div>
            )}
            ...
            <section className="flex flex-wrap items-center gap-2">
              <Link to="/pre-eval/teoria" className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white">📊 Abrir pre-evaluación</Link>
              <Link to="/buscar?materia=teoria" className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-white border hover:bg-slate-50">🔎 Buscar apuntes</Link>
