...
/* ---------- dominio: Teoría de la Computación ---------- */
const SUBJECT_SLUGS_TEO = [
  'teoria-computacion', // slug principal
  'teoria',
  'tc'
];

async function getSubjectIdsTEO() {
  try {
...
async function getQuizIdTEO(expectedType /* 'uuid' | 'integer' | 'text' */) {
  if (process.env.QUIZ_TEORIA_ID?.trim()) {
    const val = process.env.QUIZ_TEORIA_ID.trim();
  ...
  const QUIZ_SLUGS = [
    'teoria-computacion', 'teoria', 'tc',
    'teoria_pre', 'teoria-computacion_pre', 'teoria-pre', 'pre-teoria-computacion'
  ];
  try {
...
/* =========================================================
   GET /api/teoria/pre-eval  (protegido)
   ========================================================= */
router.get('/pre-eval', authenticate, async (_req, res) => {
  try {
    const subjectIds = await getSubjectIdsTEO();
    let blks;
    if (subjectIds.length) {
      ...
      console.log(`[TEO /pre-eval] filtro subject_id IN, bloques=${blks.rowCount}`);
    } else {
      blks = await pool.query(
        `select id, code, titulo, subject_id
           from blocks
          where code ilike 'TC%' or code ilike 'TEO%' or code ilike 'T%U%'
          order by code asc, titulo asc`
      );
      console.log(`[TEO /pre-eval] fallback por code ILIKE, bloques=${blks.rowCount}`);
    }
    ...
    res.json({
      subject: { id: subjectId, slug: 'teoria-computacion', nombre: 'Teoría de la Computación' },
      blocks: blks.rows,
      questions: qs.rows,
      choices: chs.rows,
      openKeys: keys.rows,
    ...
    console.error('[TEO /pre-eval] error:', err);
    res.status(500).json({ error: 'No se pudo cargar la pre-evaluación' });
  }
});

/* =========================================================
   POST /api/teoria/attempts  (protegido)
   ========================================================= */
  ...
  try {
    ...
    if (hasAttemptSessions) {
      ...
      if (sessHasQuiz) {
        ...
        quizIdForSess = await getQuizIdTEO(quizType);
        if (sessQuizNotNull && (quizIdForSess === null || typeof quizIdForSess === 'undefined')) {
          return res.status(500).json({
            error: 'attempt_sessions.quiz_id es NOT NULL y no se resolvió. Define QUIZ_TEORIA_ID o crea quiz con slug de teoría.',
          });
        }
      }
    ...
    console.error('[TEO /attempts] error:', err);
    res.status(500).json({ error: 'No se pudieron guardar los intentos' });
  }
});

/* =========================================================
   GET /api/teoria/route/summary  (protegido)
   ========================================================= */
router.get('/route/summary', authenticate, async (req, res) => {
  ...
  try {
    ...
    const quizId = await getQuizIdTEO(quizType);
    if (!quizId) return res.status(500).json({ error: 'No se pudo resolver quiz_id (QUIZ_TEORIA_ID / slug).' });
    ...
    console.error('[TEO /route/summary] error:', err);
    res.status(500).json({ error: 'No se pudo generar el resumen' });
  }
});

/* =========================================================
   GET /api/teoria/results/me  (protegido)
   ========================================================= */
router.get('/results/me', authenticate, async (req, res) => {
  ...
  try {
    ...
    console.error('[TEO /results/me] error:', err);
    res.status(500).json({ error: 'No se pudieron obtener resultados' });
  }
});

/* =========================================================
   GET /api/teoria/route/resources  (protegido)
   ========================================================= */
router.get('/route/resources', authenticate, async (req, res) => {
  ...
  try {
    ...
    console.error('[TEO /route/resources] error:', err);
