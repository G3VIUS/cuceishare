...
/* ---------- dominio: Minería de Datos ---------- */
const SUBJECT_SLUGS_MIN = [
  'mineria-datos',  // slug principal
  'mineria',        // alias
  'md'              // alias
];

async function getSubjectIdsMIN() {
  try {
...
async function getQuizIdMIN(expectedType /* 'uuid' | 'integer' | 'text' */) {
  if (process.env.QUIZ_MINERIA_ID?.trim()) {
    const val = process.env.QUIZ_MINERIA_ID.trim();
  ...
  const QUIZ_SLUGS = [
    'mineria-datos', 'mineria', 'md',
    'mineria-datos_pre', 'mineria_pre', 'md_pre',
    'mineria-datos-pre', 'pre-mineria-datos'
  ];
  try {
...
/* =========================================================
   GET /api/mineria/pre-eval  (protegido)
   ========================================================= */
router.get('/pre-eval', authenticate, async (_req, res) => {
  try {
    const subjectIds = await getSubjectIdsMIN();
    let blks;
    if (subjectIds.length) {
      ...
      console.log(`[MINERIA /pre-eval] filtro subject_id IN, bloques=${blks.rowCount}`);
    } else {
      // Fallback por code si aún no asignaste subject_id a los bloques
      blks = await pool.query(
        `select id, code, titulo, subject_id
           from blocks
          where code ilike 'MD%' or code ilike 'MIN%'
          order by code asc, titulo asc`
      );
      console.log(`[MINERIA /pre-eval] fallback por code ILIKE, bloques=${blks.rowCount}`);
    }
    ...
    res.json({
      subject: { id: subjectId, slug: 'mineria-datos', nombre: 'Minería de Datos' },
      blocks: blks.rows,
      questions: qs.rows,
      choices: chs.rows,
      openKeys: keys.rows,
    ...
    console.error('[MINERIA /pre-eval] error:', err);
    res.status(500).json({ error: 'No se pudo cargar la pre-evaluación' });
  }
});

/* =========================================================
   POST /api/mineria/attempts  (protegido)
   ========================================================= */
  ...
  try {
    ...
    if (hasAttemptSessions) {
      ...
      if (sessHasQuiz) {
        ...
        quizIdForSess = await getQuizIdMIN(quizType);
        if (sessQuizNotNull && (quizIdForSess === null || typeof quizIdForSess === 'undefined')) {
          return res.status(500).json({
            error: 'attempt_sessions.quiz_id es NOT NULL y no se resolvió. Define QUIZ_MINERIA_ID o crea quiz con slug minería.',
          });
        }
      }
    ...
    async function insertOption({ blockId, questionId, choiceId }) {
      ...
      if (canAutoGrade) {
        ...
        // Marca correcto si el choice pertenece a la pregunta y es correcto (correct o es_correcta)
        vals.push(`exists (
          select 1
            from choices ch
           where ch.id=${cp}
    ...
    console.error('[MINERIA /attempts] error:', err);
...
/* =========================================================
   GET /api/mineria/route/summary  (protegido)
   ========================================================= */
router.get('/route/summary', authenticate, async (req, res) => {
  ...
  try {
    ...
    const quizId = await getQuizIdMIN(quizType);
    if (!quizId) return res.status(500).json({ error: 'No se pudo resolver quiz_id (QUIZ_MINERIA_ID / slug).' });
    ...
    console.error('[MINERIA /route/summary] error:', err);
    res.status(500).json({ error: 'No se pudo generar el resumen' });
  }
});

/* =========================================================
   GET /api/mineria/results/me  (protegido)
   ========================================================= */
router.get('/results/me', authenticate, async (req, res) => {
  ...
  try {
    ...
    console.error('[MINERIA /results/me] error:', err);
    res.status(500).json({ error: 'No se pudieron obtener resultados' });
  }
});

/* =========================================================
   GET /api/mineria/route/resources  (protegido)
   ========================================================= */
router.get('/route/resources', authenticate, async (req, res) => {
  ...
  try {
    ...
    console.error('[MINERIA /route/resources] error:', err);
