...
const SUBJECT_SLUGS = [
  'ingenieria-software', // principal (DB)
  'ingsoft',             // alias corto
  'isw', 'software'      // otros alias opcionales
];
...
async function getQuizId(expectedType /* 'uuid' | 'integer' | 'text' */) {
  if (process.env.QUIZ_INGSOFT_ID?.trim()) {
    const val = process.env.QUIZ_INGSOFT_ID.trim();
  ...
  const QUIZ_SLUGS = [
    'ingenieria-software', 'ingsoft', 'isw', 'software',
    'ingsoft_pre', 'isw_pre', 'pre-ingsoft', 'pre-isw'
  ];
  try {
...
/* =========================================================
   GET /api/ingsoft/pre-eval  (protegido)
   ========================================================= */
router.get('/pre-eval', authenticate, async (_req, res) => {
  try {
    ...
    if (subjectIds.length) {
      ...
      // Fallback por code si no asignaste subject_id (prefijo ISW-)
      blks = await pool.query(
        `select id, code, titulo, subject_id
           from blocks
          where code ilike 'ISW-%'
          order by coalesce(orden, 999999), code asc, titulo asc`
      );
    }
    ...
    res.json({
      subject: { id: subjectId, slug: 'ingsoft', nombre: 'Ingeniería de Software' },
      blocks: blks.rows,
      questions: qs.rows,
      choices: chs.rows,
      openKeys: keys.rows,
    ...
    console.error('[INGSOFT /pre-eval] error:', err);
    res.status(500).json({ error: 'No se pudo cargar la pre-evaluación' });
  }
});

/* =========================================================
   POST /api/ingsoft/attempts  (protegido)
   ========================================================= */
  ...
  try {
    ...
    if (hasAttemptSessions) {
      ...
      if (sessHasQuiz) {
        ...
        quizIdForSess = await getQuizId(quizType);
        if (sessQuizNotNull && (quizIdForSess === null || typeof quizIdForSess === 'undefined')) {
          return res.status(500).json({
            error: 'attempt_sessions.quiz_id es NOT NULL y no se resolvió. Define QUIZ_INGSOFT_ID o crea quiz con slug ingenieria-software.',
          });
        }
      }
    ...
    console.error('[INGSOFT /attempts] error:', err);
    res.status(500).json({ error: 'No se pudieron guardar los intentos' });
  }
});

/* =========================================================
   GET /api/ingsoft/route/summary  (protegido)
   ========================================================= */
router.get('/route/summary', authenticate, async (req, res) => {
  ...
  try {
    ...
    const quizId = await getQuizId(quizType);
    if (!quizId) return res.status(500).json({ error: 'No se pudo resolver quiz_id (QUIZ_INGSOFT_ID / slug).' });
    ...
    console.error('[INGSOFT /route/summary] error:', err);
    res.status(500).json({ error: 'No se pudo generar el resumen' });
  }
});

/* =========================================================
   GET /api/ingsoft/results/me  (protegido)
   ========================================================= */
router.get('/results/me', authenticate, async (req, res) => {
  ...
  try {
    ...
    console.error('[INGSOFT /results/me] error:', err);
    res.status(500).json({ error: 'No se pudieron obtener resultados' });
  }
});

/* =========================================================
   GET /api/ingsoft/route/resources  (protegido)
   ========================================================= */
router.get('/route/resources', authenticate, async (req, res) => {
  ...
  try {
    ...
    console.error('[INGSOFT /route/resources] error:', err);
