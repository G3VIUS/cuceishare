...
/* ---------- dominio: Redes ---------- */
const SUBJECT_SLUGS_RED = [
  'redes',     // principal
  'rn',        // alias opcional
  'red'        // alias opcional
];

async function getSubjectIdsRED() {
  try {
...
async function getQuizIdRED(expectedType /* 'uuid' | 'integer' | 'text' */) {
  if (process.env.QUIZ_REDES_ID?.trim()) {
    const val = process.env.QUIZ_REDES_ID.trim();
...
/* =========================================================
   GET /api/redes/pre-eval  (protegido)
   ========================================================= */
router.get('/pre-eval', authenticate, async (_req, res) => {
  try {
    const subjectIds = await getSubjectIdsRED();
    ...
    if (subjectIds.length) {
      ...
      // Fallback por code si no asignaste subject_id
      blks = await pool.query(
        `select id, code, titulo, subject_id
           from blocks
          where code ilike 'RN%' or code ilike 'RED%'
          order by coalesce(orden, 999999), code asc, titulo asc`
      );
    }
    ...
    res.json({
      subject: { id: subjectId, slug: 'redes', nombre: 'Redes' },
      blocks: blks.rows,
      questions: qs.rows,
      choices: chs.rows,
      openKeys: keys.rows,
    ...
    console.error('[REDES /pre-eval] error:', err);
    res.status(500).json({ error: 'No se pudo cargar la pre-evaluación' });
  }
});

/* =========================================================
   POST /api/redes/attempts  (protegido)
   ========================================================= */
  ...
  try {
    ...
    if (hasAttemptSessions) {
      ...
      if (sessHasQuiz) {
        ...
        quizIdForSess = await getQuizIdRED(quizType);
        if (sessQuizNotNull && (quizIdForSess === null || typeof quizIdForSess === 'undefined')) {
          return res.status(500).json({
            error: 'attempt_sessions.quiz_id es NOT NULL y no se resolvió. Define QUIZ_REDES_ID o crea quiz con slug redes.',
          });
        }
      }
    ...
    const canAutoGrade = await tableHasColumn('choices', 'correct'); // existe en tu BDD
    ...
    async function insertOption({ blockId, questionId, choiceId }) {
      ...
      if (canAutoGrade) {
        ...
        // usa coalesce(correct, es_correcta)
        vals.push(`exists (
    ...
    console.error('[REDES /attempts] error:', err);
    res.status(500).json({ error: 'No se pudieron guardar los intentos' });
  }
});

/* =========================================================
   GET /api/redes/route/summary  (protegido)
   ========================================================= */
router.get('/route/summary', authenticate, async (req, res) => {
  ...
  try {
    ...
    const quizId = await getQuizIdRED(quizType);
    if (!quizId) return res.status(500).json({ error: 'No se pudo resolver quiz_id (QUIZ_REDES_ID / slug).' });
    ...
    console.error('[REDES /route/summary] error:', err);
    res.status(500).json({ error: 'No se pudo generar el resumen' });
  }
});

/* =========================================================
   GET /api/redes/results/me  (protegido)
   ========================================================= */
router.get('/results/me', authenticate, async (req, res) => {
  ...
  try {
    ...
    console.error('[REDES /results/me] error:', err);
    res.status(500).json({ error: 'No se pudieron obtener resultados' });
  }
});

/* =========================================================
   GET /api/redes/route/resources  (protegido)
   ========================================================= */
router.get('/route/resources', authenticate, async (req, res) => {
  ...
  try {
    ...
    console.error('[REDES /route/resources] error:', err);
