...
const SUBJECT_SLUGS_SI = [
  'seginf',                    // principal recomendado
  'seguridad-info',            // alias
  'seguridad-informacion',     // alias
  'seguridad-de-la-informacion', // alias
  'infosec', 'si'              // opcionales
];

async function getSubjectIdsSI() {
  try {
...
async function getQuizIdSI(expectedType /* 'uuid' | 'integer' | 'text' */) {
  if (process.env.QUIZ_SEGINF_ID?.trim()) {
    const val = process.env.QUIZ_SEGINF_ID.trim();
  ...
  const QUIZ_SLUGS = [
    'seginf', 'seguridad-info', 'seguridad-informacion', 'seguridad-de-la-informacion', 'infosec', 'si',
    'seginf_pre', 'seguridad-info_pre', 'seguridad-informacion_pre',
    'seginf-pre', 'pre-seginf'
  ];
  try {
...
/* =========================================================
   GET /api/seginf/pre-eval  (protegido)
   ========================================================= */
router.get('/pre-eval', authenticate, async (_req, res) => {
  try {
    const subjectIds = await getSubjectIdsSI();
    ...
    if (subjectIds.length) {
      blks = await pool.query(
        `select id, code, titulo, subject_id
          ...
          order by coalesce(orden, 999999), code asc, titulo asc`,
        [subjectIds]
      );
      console.log(`[SEGINF /pre-eval] filtro subject_id IN, bloques=${blks.rowCount}`);
    } else {
      // Fallback por code si aún no asignaste subject_id a los bloques
      blks = await pool.query(
        `select id, code, titulo, subject_id
           from blocks
          where code ilike 'SI%' or code ilike 'SEG%'
          order by coalesce(orden, 999999), code asc, titulo asc`
      );
      console.log(`[SEGINF /pre-eval] fallback por code ILIKE, bloques=${blks.rowCount}`);
    }
    ...
    if (qIds.length) {
      chs = await pool.query(
        `select id, question_id, texto, correct, es_correcta
           from choices
    ...
    // Tomamos el primero si hay varios sujetos con alias
    const subjectId = subjectIds[0] || null;

    res.json({
      subject: { id: subjectId, slug: 'seginf', nombre: 'Seguridad de la Información' },
      blocks: blks.rows,
      questions: qs.rows,
      choices: chs.rows,
      openKeys: keys.rows,
    ...
    console.error('[SEGINF /pre-eval] error:', err);
    res.status(500).json({ error: 'No se pudo cargar la pre-evaluación' });
  }
});

/* =========================================================
   POST /api/seginf/attempts  (protegido)
   ========================================================= */
  ...
  try {
    ...
    if (hasAttemptSessions) {
      ...
      if (sessHasQuiz) {
        ...
        quizIdForSess = await getQuizIdSI(quizType);
        if (sessQuizNotNull && (quizIdForSess === null || typeof quizIdForSess === 'undefined')) {
          return res.status(500).json({
            error: 'attempt_sessions.quiz_id es NOT NULL y no se resolvió. Define QUIZ_SEGINF_ID o crea quiz con slug seginf.',
          });
        }
      }
    ...
    const hasCorrectCol = await tableHasColumn('attempts', 'correct');
    ...
    async function insertOption({ blockId, questionId, choiceId }) {
      if (!choiceId) {
        throw Object.assign(new Error('Falta choiceId para una respuesta de tipo opcion'), { statusCode: 400 });
      }
    ...
    console.error('[SEGINF /attempts] error:', err);
    const status = err?.statusCode || 500;
    res.status(status).json({ error: err?.message || 'No se pudieron guardar los intentos' });
  }
});

/* =========================================================
   GET /api/seginf/route/summary  (protegido)
   ========================================================= */
router.get('/route/summary', authenticate, async (req, res) => {
  ...
  try {
    ...
    const quizId = await getQuizIdSI(quizType);
    if (!quizId) return res.status(500).json({ error: 'No se pudo resolver quiz_id (QUIZ_SEGINF_ID / slug).' });
    ...
    const { rows } = await pool.query(
      ...
      select
        ...
        count(*) filter (where last_attempt.correct is true) as correct_option,
        0                                                    as total_open
      from last_attempt
      join blocks b on b.id = last_attempt.block_id
    ...
    console.error('[SEGINF /route/summary] error:', err);
    res.status(500).json({ error: 'No se pudo generar el resumen' });
  }
});

/* =========================================================
   GET /api/seginf/results/me  (protegido)
   ========================================================= */
router.get('/results/me', authenticate, async (req, res) => {
  ...
  try {
    ...
    const agg = await pool.query(
      ...
      select
        count(*)                                as total_option,
        count(*) filter (where correct is true) as correct_option
      from last_attempt
      `,
      [sessionId]
    );
    ...
    try {
      const bd = await pool.query(
        ...
        select
          q.dificultad,
          count(*)                                               as total,
          count(*) filter (where last_attempt.correct is true)   as correctas,
          case when count(*) = 0 then 0
               else round(100.0 * count(*) filter (where last_attempt.correct is true) / count(*), 2)
          end as porcentaje
        from last_attempt
        join questions q on q.id = last_attempt.question_id
    ...
    console.error('[SEGINF /results/me] error:', err);
    res.status(500).json({ error: 'No se pudieron obtener resultados' });
  }
});

/* =========================================================
   GET /api/seginf/route/resources  (protegido)
   ========================================================= */
router.get('/route/resources', authenticate, async (req, res) => {
  ...
  try {
    ...
    console.error('[SEGINF /route/resources] error:', err);
