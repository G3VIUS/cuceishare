...
const SUBJECT_SLUGS_PROG = [
  'programacion',   // principal
  'prog',           // alias opcional
  'programa'        // alias opcional
];

async function getSubjectIdsPROG() {
  try {
...
async function getQuizIdPROG(expectedType /* 'uuid' | 'integer' | 'text' */) {
  if (process.env.QUIZ_PROGRAMACION_ID?.trim()) {
    const val = process.env.QUIZ_PROGRAMACION_ID.trim();
  ...
  const QUIZ_SLUGS = [
    'programacion', 'prog',
    'programacion_pre', 'programacion-pre', 'pre-programacion', 'prog-pre'
  ];
  try {
...
/* =========================================================
   GET /api/programacion/pre-eval  (protegido)
   ========================================================= */
router.get('/pre-eval', authenticate, async (_req, res) => {
  try {
    const subjectIds = await getSubjectIdsPROG();
    ...
    if (subjectIds.length) {
      ...
      // Fallback por code si aún no asignas subject_id en blocks
      blks = await pool.query(
        `select id, code, titulo, subject_id
           from blocks
          where code ilike 'PRG%' or code ilike 'PROG%'
          order by coalesce(orden, 999999), code asc, titulo asc`
      );
    }
    ...
    res.json({
      subject: { id: subjectId, slug: 'programacion', nombre: 'Programación' },
      blocks: blks.rows,
      questions: qs.rows,
      choices: chs.rows,
      openKeys: keys.rows,
    ...
    console.error('[PROGRAMACION /pre-eval] error:', err);
    res.status(500).json({ error: 'No se pudo cargar la pre-evaluación' });
  }
});

/* =========================================================
   POST /api/programacion/attempts  (protegido)
   ========================================================= */
  ...
  try {
    ...
    if (hasAttemptSessions) {
      ...
      if (sessHasQuiz) {
        ...
        quizIdForSess = await getQuizIdPROG(quizType);
        if (sessQuizNotNull && (quizIdForSess === null || typeof quizIdForSess === 'undefined')) {
          return res.status(500).json({
            error: 'attempt_sessions.quiz_id es NOT NULL y no se resolvió. Define QUIZ_PROGRAMACION_ID o crea quiz con slug programacion.',
          });
        }
      }
    ...
    const canAutoGrade = await tableHasColumn('choices', 'correct');
    ...
    console.error('[PROGRAMACION /attempts] error:', err);
    res.status(500).json({ error: 'No se pudieron guardar los intentos' });
  }
});

/* =========================================================
   GET /api/programacion/route/summary  (protegido)
   ========================================================= */
router.get('/route/summary', authenticate, async (req, res) => {
  ...
  try {
    ...
    const quizId = await getQuizIdPROG(quizType);
    if (!quizId) return res.status(500).json({ error: 'No se pudo resolver quiz_id (QUIZ_PROGRAMACION_ID / slug).' });
    ...
    console.error('[PROGRAMACION /route/summary] error:', err);
    res.status(500).json({ error: 'No se pudo generar el resumen' });
  }
});

/* =========================================================
   GET /api/programacion/results/me  (protegido)
   ========================================================= */
router.get('/results/me', authenticate, async (req, res) => {
  ...
  try {
    ...
    console.error('[PROGRAMACION /results/me] error:', err);
    res.status(500).json({ error: 'No se pudieron obtener resultados' });
  }
});

/* =========================================================
   GET /api/programacion/route/resources  (protegido)
   ========================================================= */
router.get('/route/resources', authenticate, async (req, res) => {
  ...
  try {
    ...
    console.error('[PROGRAMACION /route/resources] error:', err);
